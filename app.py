...
def _run_query(db: SQLDatabase, query: str) -> str:
    """Execute SQL and return results as a string. Works across all langchain versions."""
    try:
        return db.run(query)
    except Exception as e:
        return f"Error: {e}"
...
def ask_database(question: str, db: SQLDatabase, llm: ChatGroq) -> dict:
    ...
    try:
        # Attempt 1
        query = _generate_sql(llm, dialect, schema, question)
        raw_result = _run_query(db, query)

        # Retry once if execution returned an error string
        if _is_sql_error(raw_result):
            query = _generate_sql(llm, dialect, schema, question, prior_error=raw_result)
            raw_result = _run_query(db, query)
